...
// this is to upload  an imageand store it in your local  device 
...
router.post("/add_students", upload.single("image"), (req, res) => {
  const sql = `INSERT INTO students (name,email,password,address,department,image,category_id) VALUES(?)`;
...
router.put("/edit_students/:id", (req, res) => {
  ...
  const sql = `UPDATE students
   set name=?, email=?,department=?,address=?,
...
router.get('/department_count',(req, res) => {
  const sql="select count(department) as  department from students"
  con.query(sql, (err, result) => {
    if (err) return res.json({ Status: false, Error: "Query Error" + err });
    return res.json({ Status: true, Result: result });
  });
